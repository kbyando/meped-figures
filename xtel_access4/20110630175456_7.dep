...
;        and determine eligibility for a line-of-sight (straight-line)
;	 trajectory through the instrument field of view.
;
; CALLING SEQUENCE:
;	G4_TRACEFOV(position3, momentum3, typeCode, [INDICES=range])
;
; DESCRIPTION:
;       02/25/2011 :: G4_TRACEFOV will return a 1-D vector of 
;        type BOOLEAN corresponding to the optional indexing 
;        parameter RANGE.  Values "1" indicate that a straight-
;	 -line particle trajectory will propagate through the 
;	 collimator in the usual sense (positive time, forwards 
;	 velocity); values "0" indicate that the propagated 
;	 particle trajectory fails to meet necessary criteria.
; 
; INPUTS:
...
;
; OUTPUTS:
;       If successful, G4_TRACEFOV will return a truth vector of 
;         type BOOLEAN, for subsequent use with the WHERE() function.
;
; SEE ALSO:
;       G4_TRACE2APER.PRO - "raytrace" for (loose) FOV requirements
;       G4_ANALYZEFOV.PRO - use of G4_TRACEFOV.PRO
;
; DEPENDENCES:
;       (NONE)
;
...
;       v1.0 adapted from G4_DISCERNFOV.PRO
;-


FUNCTION g4_tracefov, pos3, vel3, typecode, INDICES=range
...
IF (typeCode EQ 1) THEN BEGIN		; p-tel specific
	x_oaperture = (-0.6755)*25.4	; [mm] placement of outer aperture
	x_iaperture = (0.1755)*25.4	; [mm] placement of inner aperture
ENDIF ELSE IF (typeCode EQ 2) THEN BEGIN ; e-tel specific
	x_oaperture = (-0.6255)*25.4	; [mm] placement of outer aperture
	x_iaperture = (0.2255)*25.4	; [mm] placement of inner aperture
ENDIF ELSE RETURN, -1		; fail
; Now, consider shared specifications
				; shared specifications
r_oaperture = (0.328/2.)*25.4	; [mm] radius of outer aperture
r_iaperture = (0.130/2.)*25.4 	; [mm] radius of inner aperture
...
time2o = (x_oaperture - pos3[0,range])/vel3[0,range]
 time2i = (x_iaperture - pos3[0,range])/vel3[0,range]
...
z_final_o = vel3[2,range]*time2o + pos3[2,range]	; (z-coordinate)
 y_final_i = vel3[1,range]*time2i + pos3[1,range]	; (y-coordinate)
 z_final_i = vel3[2,range]*time2i + pos3[2,range]	; (z-coordinate)
							; (x-coords known)
; 3. conditional testing
; 3a. test for passage through bottleneck
pass_thru_o = (y_final_o^2. + z_final_o^2.) LE (r_oaperture^2.)
pass_thru_i = (y_final_i^2. + z_final_i^2.) LE (r_iaperture^2.)
; 3b. require passage in positive time
positivet_o = (time2o GT 0.)
positivet_i = (time2i GT 0.)
; 3c. require passage through outer aperture before inner aperture (positive velocity)
into_scope = vel3[0,range] GT 0.
;	note: returns arrays of 1s (TRUE) and 0s (FALSE)

; 4. field-of-view (FOV) defined by passage through *both* apertures in positive time
;  and appropriate direction
fov = pass_thru_o * pass_thru_i * positivet_o * positivet_i * into_scope
